...
#define SAVE_FILE_SUFFIX ".txt"  // file to store reading data
#define DATA_START 5
#define DATA_STOP 6

#define MAX_SAMPLES 100
#define MAX_LOOP_SAMPLES 100
char SAVE_FILEPATH[100];
char SAVE_FILE[50];
char DUT_ID[20];
FILE* file_pointer;
...
long int Parse_On_Time(const char* filepath)
{
...
// Searches only files with todays date that have the same COMPRESSOR_ID string
long int Search_Compressor_On_Time(const char* COMPRESSOR)
{
    long int on_time = 0;
    char file_result[50];     // temp buffer to store all filenames in Log directory
    ...
    USB_directory = opendir(USB_FILEPATH);
    if (USB_directory)    // this evaluates to true if a USB drive is installed
    {
        while ( (directory_entry = readdir(USB_directory) ) != NULL)    // Check each "file entry" in the specified directory
        {
            //printf("ANY_FILE:\t\t%s\n", directory_entry->d_name);   // these could be files or directories or symbolic links or socket or pipe
            if(directory_entry->d_type == DT_REG)
            {
                printf("REG_FILE:\t\t%s\n", directory_entry->d_name);
                file_result[0] = '\0';  // make sure string is empty before performing operations on it
                strcpy(file_result, directory_entry->d_name);

                bool contains = strstr(file_result, SAVE_FILE);
                if(contains)
                {
                    printf("A match has been found!\n");
                    Parse_On_Time(SAVE_FILEPATH);

                }

            }
        }
        closedir(USB_directory);
     }
     else
     {
       SD_directory = opendir(SD_FILEPATH);
       if (SD_directory)
       {
           while ( (directory_entry = readdir(SD_directory) ) != NULL)
           {
               printf("ANY_FILE:\t\t%s\n", directory_entry->d_name);
  ...
  while( (fgets(line, 256, file_pointer) != NULL) && lines_read <10 && !Last_Item)
	{
      ...
      contains = strstr(line, BLACKBOX_TAG);
			if(contains)
			{
        sscanf(line, "%s %s ", temp, BLACKBOX_ID );  // parse line and store the second token in BLACKBOX
        continue;
			}
      ...
      contains = strstr(line, DISPLACER_TAG);
			if(contains)
			{
        sscanf(line, "%s %s ", temp, DISPLACER_ID );  // parse line and store the second token in DISPLACER, first token is discarded
        continue;
			}
      ...
      contains = strstr(line, CCC_TAG);
			if(contains)
			{
        sscanf(line, "%s %s ", temp, CCC_ID );  // store the second string token in CCC
        continue;
			}
...
void Open_Log_File()
{
    ...
    DATE_TODAY[0] = '\0';
    strcpy(DATE_TODAY, date_buffer);
    DATE_TODAY[strlen(DATE_TODAY)] = '\0';

    sprintf(SAVE_FILE, "%s_%s_%s", date_buffer, COMPRESSOR_ID, SAVE_FILE_SUFFIX);
    ...
    if(thumb_drive_exists==0)sprintf(fileName, "%s/%s_%s_%s", USB_FILEPATH, date_buffer, COMPRESSOR_ID, SAVE_FILE_SUFFIX);         // save logfile on USB Drive
    else sprintf(fileName, "%s/%s_%s_%s", SD_FILEPATH, date_buffer, COMPRESSOR_ID, SAVE_FILE_SUFFIX);                // else on SD card
    strcpy(SAVE_FILEPATH, fileName);
    printf("save filepath:  %s\n", SAVE_FILEPATH);
    printf("save file:  %s\n", SAVE_FILE);

    file_pointer = fopen(SAVE_FILEPATH, "a");
    fclose( file_pointer );
}
...
//	Possibly Check if file exists already for todays date, if so dont overwrite file
		// Initialize data storage indices to 0
    Clear_Loop_Index();
    ...
    char* file_header2 = "   V \t     A \t       W        degK     diodeV      V\t    mA\t    mW \t\t C \t  C \t  C \t  Time\t   Date\t\tElapsed_Time \t      Relay";
    char* line = "-----------------------------------------------------------------------------------------------------------------------------------------------------------\n";
    ...
    long int cool_on_time = Search_Compressor_On_Time(COMPRESSOR_ID);
    ...
    Dewar_SN[strlen(Dewar_SN)] = '\0';
	  ...
	  file_pointer = fopen(SAVE_FILEPATH, "a"); // open for writing, 'w' means overwrite any existing file, "a" means to append to file
...
void Record_Relay_ON()
{
	...
	file_pointer = fopen(SAVE_FILEPATH, "a"); // 'w' means overwrite any existing file, "a" means to append to file
